...
def test_simple():
    run_valid_parse_test('a | b', Choice.parse)
...
def test_end_of_file():
    ...
    run_invalid_parse_test('a |', Choice.parse, errors)
...
def test_empty_option():
    run_invalid_parse_test('a || b', Choice.parse, [('empty option', (1, 3))])

    errors = [
        ('empty option', (1, 3)), 
        ('empty option', (1, 5))
    ]
    run_invalid_parse_test('a | || b', Choice.parse, errors)


def test_duplicate_option():
    run_invalid_parse_test('a | a', 
            Choice.parse, 
            [('duplicate option', (1, 5))])